function ajax_listar(){
   ...
   .done( function(resp) {
	...
	.fail( function(request) {
		toastr.error('No se pudo ejecutar "ajax:/equipos/listar"\n'+request.responseText);
   });
}
...
function ajax_crear() {
   ...
   .done( function (resp) {
      if ( resp == '' )
         toastr.error('Hubo un error al intentar crear el equipo.');
      else 
			toastr.success('Se creó el equipo.');
   })
   .fail( function (request) {
   	toastr.error('No se pudo ejecutar "ajax:/equipos/crear"\n'+request.responseText);
   })
}
 	...
 	.done( function (resp) {
      if ( resp == 0 )
         toastr.error('Hubo un error al intentar modificar.');
			...
			toastr.success('Se modificó el equipo.');
   })
   .fail( function (request) {
      toastr.error('No se pudo ejecutar "ajax:/equipos/modificar"\n'+request.responseText);
   })
}
...
function ajax_eliminar() {
   ...
   .done( function (resp) {
         if ( resp == '' )
            toastr.error('Hubo un error al intentar eliminar el equipo.');
         else 
				toastr.success('Se eliminó el equipo correctamente.');
   })
   .fail( function (request) {
         toastr.error('No se pudo ejecutar "ajax:/equipos/eliminar"\n'+request.responseText);
   })
}
...
function ajax_consultar() {
   ...
   .done( function (resp) {
		if ( resp == '' )
         toastr.error('Hubo un error al intentar consultar el equipo.');
   })
   .fail( function (request) {
      toastr.error('No se pudo ejecutar "ajax:/equipos/consultar"\n'+request.responseText);
   })
}
